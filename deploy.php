...
option(
    ...
    'Всегда пропускаем проверку на незакоммиченные и не запушенные изменения на деплой'
);
...
set('restart_cmd', 'sudo /usr/sbin/service httpd restart');
...
set('sync', [
    'files' => [
		...
		// TODO: продолжить при необходимости по аналогии
    ]
]);
...
task('check:uncommited', function() {
    ...
    if ($skipCheck) {
        writeln('<info>Проверка на незакоммиченные изменения будет пропущена</info>');
        return;
    }
    ...
    if (strlen($result) > 0) {
        ...
        throw new \RuntimeException(
            "Рабочая копия содержит незакоммиченные/незастейженные изменения"
        );
    }

    else {
        writeln('<info>Рабочая копия не содержит незакоммиченных/незастейженных изменений</info>');
    }
});
...
task('check:unpushed', function() {
    ...
    if ($skipCheck) {
        writeln('<info>Проверка на незапушенные изменения будет пропущена</info>');
        return;
    }
    ...
    if (strlen($result) > 0) {
        ...
        throw new \RuntimeException(
            "Рабочая копия содержит незапушенные изменения"
        );
    }

    else {
        writeln('<info>Рабочая копия НЕ содержит незапушенные изменения</info>');
    }
});
